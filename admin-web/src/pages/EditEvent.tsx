...
export default function EditEvent() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [eventData, catsData, placesData] = await Promise.all([
        adminEventsService.getById(id!),
        supabase.from('activity_categories').select('*').order('name_en'),
      ...
      setFormData({
        ...eventData,
        event_start_datetime: eventData.event_start_datetime?.slice(0, 16) || '',
        event_end_datetime: eventData.event_end_datetime?.slice(0, 16) || '',
      });

      setCategories(catsData.data || []);
      setPlaces(placesData.data || []);
      setSelectedCategories(eventData.categories?.map((c: any) => c.category_id) || []);
      setUseCustomLocation(!eventData.place_id);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await adminEventsService.update(id, eventData, selectedCategories);
